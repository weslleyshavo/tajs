...
describe("Unit tests for frontend", () => {
  ...
  it("should verify either all functions were called properly", async () => {
    ...
    const view = new View();
    view.updateTable = tracker.calls(view.updateTable); // encapsula no tracker para testar as chamadas
    ...
    const [{ arguments: serviceCalls }] = tracker.getCalls(
      service.getCharacters
    );
    deepStrictEqual(serviceCalls, [{ skip: 0, limit: 5 }]);

    const [{ arguments: viewCalls }] = tracker.getCalls(view.updateTable);
    deepStrictEqual(viewCalls, [
      [
        { isBold: true, name: "morty smith", image: "https://" },
        { isBold: false, name: "pickle rick", image: "https://" },
      ],
    ]);
